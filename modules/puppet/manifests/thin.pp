...
class puppet::thin(
    ...
    $puppet_socket = '/var/run/puppet/puppetmasterd.sock',
    $puppet_workers = '4',
    ...
    supervisord::program {
      'puppetmaster':
        command => "${thin} start -P ${puppet_pid} -e production --servers ${puppet_workers} --daemonize --socket ${puppet_socket}--chdir ${puppet_dir}  --user puppet --group puppet -R ${puppet_rack_config}",
        cwd     => '/usr/bin',
        user    => 'puppet',
        require => [
